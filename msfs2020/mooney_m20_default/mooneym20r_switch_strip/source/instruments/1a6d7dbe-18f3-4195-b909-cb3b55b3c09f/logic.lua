...
--******************************************************************************************
  ...
  NOTES:
    ...
    V1.01 - Released 2022-01-03 - fixed swtich cover not always workign in sync with virtual cockpit. 
...
function cover_cb()
  if cover_state == 0 then
    ...
    fs2020_variable_write("L:Tapas_switch_hight_boost", "Number", 1)
  else
    fs2020_variable_write("L:M20R_switch_hight_boost", "Number", 0)
    fs2020_variable_write("L:Tapas_switch_hight_boost", "Number", 0)
...
function cb_set_cover_state(state)
  cover_state = state
  if cover_state == 0 then
    visible(cover_up_id, false)
    visible(cover_dn_id, true)
...
fs2020_variable_subscribe("L:Tapas_switch_hight_boost", "Number", cb_set_cover_state)
